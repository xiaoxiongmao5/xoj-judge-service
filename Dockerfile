...
RUN go build -o xoj-judge-service main.go
...
COPY --from=x86_64_builder /app/xoj-judge-service .
...
# 复制二进制文件从构建阶段的镜像到最终的镜像
COPY --from=arm64v8_builder /app/xoj-judge-service .
